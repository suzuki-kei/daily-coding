...
function generate_random_ascending_values(n: number): number[]
{
    ...
    for (let i = 0; i < n; i++)
    {
        const offset: number = (values.at(-1) || 9) + 1
...
function checkedAt<T>(values: readonly T[], i: number): T
{
    const value = values.at(i)

    if (value === undefined)
        throw new Error(`values.at(${i}) is undefined`)

    return value
}

function binary_search<T>(values: readonly T[], target: T, lower?: number, upper?: number): number
{
    lower ??= 0
