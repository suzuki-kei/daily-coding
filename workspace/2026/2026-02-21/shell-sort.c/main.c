...
void insert(int *array, int start_index, int hop);
...
void shell_sort(int *array, int n)
{
    for (int hop = compute_initial_hop(n); hop >= 1; hop /= 3)
        for (int start_index = hop; start_index < n; start_index++)
            insert(array, start_index, hop);
}
...
void insert(int *array, int start_index, int hop)
{
    int insertion_index = start_index;
    int insertion_value = array[start_index];
