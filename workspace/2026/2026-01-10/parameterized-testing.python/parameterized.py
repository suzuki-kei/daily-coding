...
def expand(expected_arguments_tuples):
    ...
    def make_expanded_methods(parameterized_method):
        return [
            make_expanded_method(parameterized_method, arguments, index)
            for index, arguments in enumerate(expected_arguments_tuples)
        ]

    def make_expanded_method(parameterized_method, arguments, index):
        expanded_method = lambda self: parameterized_method(self, *arguments)
