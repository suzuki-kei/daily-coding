...
testsWithTestDataTuples :: String -> (String -> [EncodedRun Char] -> Test) -> Test
testsWithTestDataTuples label assert =
    let
...
encodeTests :: Test
encodeTests = testsWithTestDataTuples "encode" assert
    where
        assert xs encodedRuns = encodedRuns ~=? encode xs

decodeTests :: Test
decodeTests = testsWithTestDataTuples "decode" assert
    where
        assert xs encodedRuns = xs ~=? decode encodedRuns
