...
data NonEmptyTree a =
    InternalNode {
        internalNodeValue :: a,
        internalNodeLeft  :: Tree a,
        internalNodeRight :: Tree a
    }
    deriving (Eq, Show)
...
parseNonEmptyTree (Node value left right) =
    Just (InternalNode value left right)
...
popMin (InternalNode value left right) =
        ...
        where
            ...
            popMin' (Node value left right) =
                let
                    ...
                    tree' = Node value newLeft right
                in
                    (minValue, tree')

popMax ::
    NonEmptyTree a -> (a, Tree a)
popMax (InternalNode value left right) =
        ...
        where
            toList' pop tree xs =
                case (parseNonEmptyTree tree) of
                    ...
                    Just (nonEmptyTree) ->
                        let
                            (x, nonEmptyTree') = pop nonEmptyTree
                        in
                            toList' pop nonEmptyTree' (x : xs)
