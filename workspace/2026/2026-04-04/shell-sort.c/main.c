...
void shell_sort(int *array, int n)
{
    for (int hop = compute_initial_hop(n); 1 <= hop; hop /= 3)
    {
        for (int end = hop; end < n; end++)
        {
            int insertion_index = end;
            int insertion_value = array[end];

            while (hop <= insertion_index && insertion_value < array[insertion_index - hop])
            {
