...
void demonstration(void)
{
    ...
    double_selection_sort(array, ARRAY_LENGTH(array));
    print_array(array, ARRAY_LENGTH(array));
}
...
void double_selection_sort(int *array, int n)
{
    for (int lower = 0, upper = n - 1; lower < upper; lower++, upper--)
    {
        int minimum_index = lower;
        int maximum_index = lower;
        ...
        swap(&array[lower], &array[minimum_index]);
        swap(&array[upper], &array[maximum_index]);
    }
}
