...
void merge_sort(int *array, int n)
{
    ...
    for (int chunk_size = 1; chunk_size < n; chunk_size *= 2)
    {
        ...
        {
            const int *begin1 = &input[i];
            const int *end1   = &input[min(n, i + chunk_size)];
            const int *begin2 = &input[min(n, i + chunk_size)];
            const int *end2   = &input[min(n, i + chunk_size * 2)];
            merge(&output[i], begin1, end1, begin2, end2);
        }
