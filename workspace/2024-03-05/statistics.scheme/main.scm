...
(define print-correlation-coefficient
    (lambda (xs ys)
        (print "==== 相関係数 (correlation-coefficient)")
        (print "xs = " xs)
        (print "ys = " ys)
        (print
...
(define factorial-range
    (lambda (min max)
        (factorial-range.tailrec min max 1)))

(define factorial-range.tailrec
    (lambda (min max accumulated)
        (cond
            ((> min max)
                accumulated)
            (else
                (factorial-range.tailrec
                    (+ min 1)
                    max
                    (* accumulated min))))))
