...
class Application
{
    ...
    {
        demonstration("partition 2-way", new Partition2way());
        demonstration("partition 3-way", new Partition3way());
    }
    ...
    private static void quickSort(int[] array, int lower, int upper, Partition partition)
    {
        ...
        final PartitionResult result = partition.apply(array, lower, upper);
        quickSort(array, lower, result.leftUpper(), partition);
        quickSort(array, result.rightLower(), upper, partition);
    }

    private static class Partition2way implements Partition
    {
        public PartitionResult apply(int[] array, int lower, int upper)
        {
            int incrementIndex = lower;
            int decrementIndex = upper;
            final int pivot = randomSelect(array, lower, upper);
    ...
    private static class Partition3way implements Partition
    {
        public PartitionResult apply(int[] array, int lower, int upper)
        {
            int lessEnd = lower;
