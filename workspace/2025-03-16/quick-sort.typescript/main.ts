...
{
    return [...Array(values.length - 1).keys()].every(i => values[i] <= values[i + 1])
}
...
function quick_sort_range(values: number[], lower: number, upper: number): void
{
    ...
    const pivot = random_select_range(values, lower, upper)
...
function random_select_range(values: number[], lower: number, upper: number): number
{
