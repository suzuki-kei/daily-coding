...
type Partition = (array: number[], lower: number, upper: number) => PartitionResult
...
function print_array<T>(array: readonly T[]): void
{
    if (is_sorted(array))
...
function is_sorted<T>(array: readonly T[]): boolean
{
...
function quick_sort_range(array: number[], lower: number, upper: number, partition: Partition): void
{
    ...
    quick_sort_range(array, result.right_upper, upper, partition)
}

function partition_2way<T>(array: T[], lower: number, upper: number): PartitionResult
{
    ...
    return {
        ...
        right_upper: increment_index,
    }
}

function partition_3way<T>(array: T[], lower: number, upper: number): PartitionResult
{
    let less_end = lower
    let increment_index = lower
    let decrement_index = upper
