...
func binarySearch[T cmp.Ordered](xs []T, x T) int {
    return binarySearchRange(xs, x, 0, len(xs) - 1)
}

func binarySearchRange[T cmp.Ordered](xs []T, x T, lower int, upper int) int {
    if lower > upper {
        return -1
    }
    ...
    if x < xs[center] {
        return binarySearchRange(xs, x, lower, center - 1)
    } else {
        return binarySearchRange(xs, x, center + 1, upper)
    }
}
