...
parseNonEmptyTree (Node value left right) = Just (NonEmptyTreeNode value left right)
...
popMin (NonEmptyTreeNode value left right) = popMin' (Node value left right)
    where
...
popMax :: NonEmptyTree a -> (a, Tree a)
popMax (NonEmptyTreeNode value left right) = popMax' (Node value left right)
    where
...
toList :: (NonEmptyTree a -> (a, Tree a)) -> Tree a -> [a]
toList pop tree = toList' tree []
    where
        toList' tree xs =
