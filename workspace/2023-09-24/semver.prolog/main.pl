...
demonstration1 :-
    ...
    format("bumped_major(~p) = ~p~n", [Version, BumpedMajorVersion]),

    semver_bumped_minor(BumpedMinorVersion, Version),
    format("bumped_minor(~p) = ~p~n", [Version, BumpedMinorVersion]),

    semver_bumped_patch(BumpedPatchVersion, Version),
    format("bumped_patch(~p) = ~p~n", [Version, BumpedPatchVersion]).
...
compare_version(Version1, Version2) :-
    ...
    format("compare(~p, ~p) = ~p~n", [Version1, Version2, Compared]).
