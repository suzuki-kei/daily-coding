...
void selection_sort(int *values, int size)
{
    ...
    for (target_index = 0; target_index < size - 1; target_index++)
    {
        for (i = selected_index = target_index; i < size; i++)
            if (values[i] < values[selected_index])
                selected_index = i;
        swap(values, target_index, selected_index);
    }
}
