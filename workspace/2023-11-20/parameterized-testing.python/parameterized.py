...
def expand(tuples):
    ...
    def decorator(parameterized_method):
        f_locals = inspect.currentframe().f_back.f_locals
        f_locals |= make_expanded_method_map(parameterized_method, tuples)

    def make_expanded_method_map(parameterized_method, tuples):
        return {
            expanded_method.__name__: expanded_method
            for expanded_method in make_expanded_methods(parameterized_method, tuples)
        }

    def make_expanded_methods(parameterized_method, tuples):
        return [
