...
typedef struct PartitionResult (*FnPartition)(int *array, int lower, int upper);
...
void quick_sort(int *array, int size, FnPartition partition);
void quick_sort_range(int *array, int lower, int upper, FnPartition partition);
struct PartitionResult partition_2way(int *array, int lower, int upper);
struct PartitionResult partition_3way(int *array, int lower, int upper);
...
void quick_sort(int *array, int size, FnPartition partition)
{
    ...
    quick_sort_range(array, 0, size - 1, partition);
}

void quick_sort_range(int *array, int lower, int upper, FnPartition partition)
{
    const struct PartitionResult result = partition(array, lower, upper);

    if (lower < result.left_upper)
        quick_sort_range(array, lower, result.left_upper, partition);
    if (result.right_lower < upper)
...
struct PartitionResult partition_2way(int *array, int lower, int upper)
{
    ...
    const struct PartitionResult result = {
        decrement_index,
        increment_index,
    };
