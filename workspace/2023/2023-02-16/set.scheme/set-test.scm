...
(define test.subset?
    (lambda ()
        ...
        (test* "#1" #t (subset? '() '()))
        (test* "#2" #t (subset? '() '(1)))
        (test* "#3" #t (subset? '(1 3 5) '(1 2 3 4 5)))
        (test* "#4" #f (subset? '(1) '()))
        (test* "#5" #f (subset? '(1 2 3) '(1 2)))
        (test-end)))
...
(define test.set-equal?
    (lambda ()
        ...
        (test* "#5" #t (set-equal? '(1 2) '(2 1)))
        (test* "#6" #f (set-equal? '(1 2) '(2 3)))
        (test-end)))
...
(define test.xs->set
    (lambda ()
        ...
        (test* "#4" '(1 2 3) (xs->set '(1 2 3 1 2 3)) set-equal?)
        (test-end)))
...
(define test.set-difference
    (lambda ()
        ...
        (test* "#3" '(1 2 3) (set-difference '(1 2 3) '()) set-equal?)
        (test* "#4" '(2 3) (set-difference '(1 2 3) '(1)) set-equal?)
        (test* "#5" '(3) (set-difference '(1 2 3) '(1 2)) set-equal?)
        (test* "#6" '() (set-difference '(1 2 3) '(1 2 3)) set-equal?)
        (test* "#7" '() (set-difference '(1 2 3) '(1 2 3 4)) set-equal?)
        (test-end)))
...
(define test.set-intersection
    (lambda ()
        ...
        (test* "#5" '(3 4 5) (set-intersection '(1 2 3 4 5) '(3 4 5 6 7)) set-equal?)
        (test-end)))
