...
(define test.include?
    (lambda ()
        (test-start "include?")
        (test* "#1" #f (include? 0 '(1)))
        (test* "#2" #t (include? 1 '(1)))
        (test* "#3" #f (include? 2 '(1)))
        (test* "#4" #f (include? 0 '(1 2 3)))
        (test* "#5" #t (include? 1 '(1 2 3)))
        (test* "#6" #t (include? 2 '(1 2 3)))
        (test* "#7" #t (include? 3 '(1 2 3)))
        (test* "#8" #f (include? 4 '(1 2 3)))
        (test-end)))
...
(define test.subset?
    (lambda ()
        ...
        (test* "#3" #t (subset? '(1) '(1)))
        (test* "#4" #t (subset? '() '(1 2)))
        (test* "#5" #t (subset? '(1) '(1 2)))
        (test* "#6" #t (subset? '(1 2) '(1 2)))
        (test* "#7" #f (subset? '(0) '()))
        (test* "#8" #f (subset? '(0) '(1)))
        (test* "#9" #f (subset? '(0 1) '(1)))
        (test* "#10" #f (subset? '(1 2) '(1)))
        (test* "#11" #f (subset? '(0 1 2) '(1 2)))
        (test* "#12" #f (subset? '(1 2 3) '(1 2)))
        (test* "#13" (test-error) (subset? '(1 1) '()))
        (test* "#14" (test-error) (subset? '() '(1 1)))
        (test-end)))
...
(define test.set-equal?
    (lambda ()
        ...
        (test* "#3" #t (set-equal? '(1 2) '(1 2)))
        (test* "#4" #t (set-equal? '(1 2 3) '(1 2 3)))
        (test* "#5" #f (set-equal? '() '(1)))
        (test* "#6" #f (set-equal? '(1) '()))
        (test-end)))
...
(define test.xs->set
    (lambda ()
        ...
        (test* "#4" '(1) (xs->set '(1 1 1)) set-equal?)
...
(define test.set-difference
    (lambda ()
        ...
        (test* "#2" '(1) (set-difference '(1) '()) set-equal?)
        (test* "#3" '() (set-difference '() '(1)) set-equal?)
