...
semver_bump_major(BumpedVersion, semver(Major, _, _)) :-
    is(BumpedMajor, Major + 1),
    semver(BumpedVersion, BumpedMajor, 0, 0).

semver_bump_minor(BumpedVersion, semver(Major, Minor, _)) :-
    is(BumpedMinor, Minor + 1),
    semver(BumpedVersion, Major, BumpedMinor, 0).

semver_bump_patch(BumpedVersion, semver(Major, Minor, Patch)) :-
